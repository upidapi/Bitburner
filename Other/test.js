...
async function fixMoney(ns, target, availableRam) {
    if (ns.getServerMoneyAvailable(target) != ns.getServerMaxMoney(target)) {
        ...
        const maxMul = Math.ceil(1 / moneyLeftP)
...
export function getMaxHackThreads(ns, target, avalibleRam) {
    function hackThreadsToRam(hackThreads) {
        ...
        const neededMoneyIncD = 1 / (1 - moneyStolenP) // the + 1 is for converting to desimal form (from %)

        threads.grow = ns.growthAnalyze(target, neededMoneyIncD)
        threads.grow = (threads.grow + 1) * growMargin
        ...
        threads.weaken = secInc / ns.weakenAnalyze(1)
        threads.weaken = (threads.weaken + 1) * weakenMargin
...
export async function main(ns) {
    ...
    // ns.tprintf(fromMin)
    // ns.tprintf(optimalWeakenThreads)
    // ns.tprintf(maxWeakenThreads)
    // ns.tprintf(weakenThreads)
    // ns.tprintf("------------")
    ...
    // const grow = ns.growthAnalyze(target, maxMul)
    // const secInc = ns.growthAnalyzeSecurity(grow)
    // const weaken = secInc / ns.weakenAnalyze(1)
    ...
    // ns.tprintf(moneyLeftP)
    // ns.tprintf(maxMul)
    // ns.tprintf(threads.hack)
    // ns.tprintf(threads.grow)
    // ns.tprintf(threads.weaken)
    // ns.tprintf("------------")
    ...
    ns.tprintf(ns.getServerSecurityLevel(target))
    ns.tprintf(ns.getServerMinSecurityLevel(target))

    ns.tprintf(ns.getServerMaxMoney(target))
    ns.tprintf(ns.getServerMoneyAvailable(target))

    ns.tprintf("---------")

    let secInc = 0
    secInc += ns.hackAnalyzeSecurity(threads.hack)
    ns.tprintf(ns.hackAnalyzeSecurity(threads.hack)) // correct
    
    secInc += ns.growthAnalyzeSecurity(threads.grow)
    ns.tprintf(ns.growthAnalyzeSecurity(threads.grow))
